...
export default function FeaturedProducts() {
  ...
  return (
    <section className="w-full bg-[#F5EFE7] px-4 py-14 sm:px-6 sm:py-20 md:px-8 md:py-24 lg:py-28">
      <div className="mx-auto w-full max-w-7xl">
        ...
        >
          <div className="min-w-0">
            ...
            <p className="mt-4 max-w-xl text-sm leading-6 text-[#756457] sm:text-base sm:leading-7">
              Discover our favourite pieces, carefully selected to
              bring warmth, character and beauty into your everyday life.
            </p>
          </div>
        ...
        {loading && (
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 sm:gap-6 lg:grid-cols-4">
            {[1, 2, 3, 4].map((item) => (
              ...
              >
                <div className="h-[300px] animate-pulse bg-[#EDE2D2] sm:h-[340px]" />

                <div className="space-y-3 p-4 sm:p-5">
                  <div className="h-3 w-20 animate-pulse rounded bg-[#EDE2D2]" />

                  <div className="h-5 w-3/4 animate-pulse rounded bg-[#EDE2D2]" />

                  <div className="h-5 w-20 animate-pulse rounded bg-[#EDE2D2]" />
                </div>
              </div>
            ))}
          </div>
        ...
        {!loading && products.length === 0 && (
          <div className="rounded-2xl border border-[#E5D8C7] bg-white px-5 py-12 text-center shadow-sm sm:rounded-[2rem] sm:px-6 sm:py-16">
            ...
            <p className="mt-2 text-sm text-[#756457] sm:text-base">
              Check back soon for our handpicked favourites.
            </p>
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 sm:gap-7 lg:grid-cols-4">
            {products.map((product, index) => (
              ...
              >
                ...
                >
                  {/* Image card */}
                  <div className="relative overflow-hidden rounded-2xl bg-[#EDE2D2] shadow-sm transition-all duration-500 group-hover:-translate-y-1 group-hover:shadow-xl group-hover:shadow-[#6F4E37]/10 sm:rounded-[1.75rem]">
                    ...
                    <div className="relative h-[320px] w-full overflow-hidden sm:h-[340px] md:h-[380px] lg:h-[340px]">
                      ...
                      {/* Featured badge */}
                      <span className="absolute left-3 top-3 rounded-full border border-white/50 bg-white/90 px-2.5 py-1.5 text-[9px] font-bold uppercase tracking-wider text-[#6F4E37] shadow-sm backdrop-blur-md sm:left-4 sm:top-4 sm:px-3">
                        Featured
                      </span>

                      {/* Stock badge */}
                      {product.stock > 0 && product.stock <= 5 && (
                        <span className="absolute right-3 top-3 rounded-full bg-[#6F4E37]/90 px-2.5 py-1.5 text-[9px] font-semibold text-white backdrop-blur-md sm:right-4 sm:top-4 sm:px-3">
